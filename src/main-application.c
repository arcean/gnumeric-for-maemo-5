...
static gboolean load_it(OpenLocation *ol)
{
	if(firstOpen){
		gui_file_read (ol->app, ol->file,NULL,NULL);
		firstOpen=FALSE;
	}
	else{
		g_idle_remove_by_data ((gpointer)ol);
		g_free (ol->file);
		free(ol);
	}
}
...
void cb_openUrls (void *app, int argc, gchar **argv) {  
	...
	if(firstOpen){
		...
		OpenLocation *ol = (OpenLocation *)g_malloc0 (sizeof (OpenLocation));
		ol->app=wbcg;
		ol->file=g_strdup(argv[0]);
		g_idle_add ((GSourceFunc)load_it, (gpointer)ol);
	}
	else{
		...
		gui_file_read (new_wbcg, argv[0],NULL,NULL);
	}
}
	...
	if (startup_files) {
		...
		for (i = 0;
			...
			if (wbv != NULL) {
				...
				osso_context = osso_initialize ("com.nokia.gnumeric", GNM_VERSION_FULL, TRUE, NULL);
				//con = dbus_bus_get (DBUS_BUS_SYSTEM, NULL);
				osso_return_t osso_ret;
				osso_ret = osso_mime_set_cb (osso_context, cb_openUrls, wbcg);
...
#ifdef GNM_USE_HILDON
	osso_mime_unset_cb(osso_context);
