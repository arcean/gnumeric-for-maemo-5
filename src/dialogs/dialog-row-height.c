...
{
	gint value = atoi (gtk_entry_get_text(state->spin));  
...
/* Maemo specific: SpinButtons are without arrows */
	changed_info = TRUE;
#endif
...
dialog_row_height_set_value (gint value, RowHeightState *state)
{
	...
	gtk_entry_set_text(state->spin, g_strdup_printf("%d", adj_value));
	return adj_value;
}
...
static void
cb_dialog_row_height_value_changed (G_GNUC_UNUSED GtkEntry *spinbutton,
				    RowHeightState *state)
{
	if (!state->adjusting) {
...
cb_dialog_row_height_default_check_toggled (GtkToggleButton *togglebutton, RowHeightState *state)
{
	gboolean editable;
	if (!state->adjusting) {
		if (editable = gtk_toggle_button_get_active (togglebutton)) {
			state->adjusting = TRUE;
		...
		dialog_row_height_button_sensitivity (state);
		gtk_editable_set_editable(state->spin, !editable);
	}
}
...
dialog_row_height (WBCGtk *wbcg, gboolean use_default)
{
	...
	state->spin  = GTK_ENTRY (go_gtk_builder_get_widget (state->gui, "spin"));
	...
	cb_dialog_row_height_default_check_toggled(state->default_check, state);
